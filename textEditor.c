...
struct editorSyntax {
    char *filetype; 
    char **filematch; //Each string sonctains a pattern to match a filename against
    char **keywords;
    char *singleline_comment_start; //Defines the way to start single line comment 
    char *multiline_comment_start;
    char *multiline_comment_end;
    int flags; //Whether to highlight numbers and whether to highlight strings for that filetype
};
...
typedef struct erow {
    ...
    int hl_open_comment; //Know if previous line is part of an unclosed multi-line comment
} erow;
...
void editorUpdateSyntax(erow *row)
{
    ...
    int prev_sep = 1;
    //Keeping track of whethter we are currently inside a string
    int in_string = 0;
    ...
    while (i < row->rsize)
    {
        ...
        //Make sure separator before keyword. Ex: not highlight avoidable because there is void
        if (prev_sep)
        {
            int j = 0;
            for (; keywords[j]; j++)
            {
                ...
                if (!strncmp(&row->render[i], keywords[j], klen) && is_serparator(row->render[i + klen]))
                {
...
void editorSelectSyntaxHighlight()
{
    ...
    for (unsigned int j = 0; j < HLDB_ENTRIES; j++)
    {
        ...
        while (s->filematch[i])
        {
            ...
            //Check if filename ends with that extension. If it's not a file extension pattern, then just check to see if pattern exsits anywhere in the filename
